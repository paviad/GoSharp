...
namespace Go {
    ...
    public class Game {
        ...
        private static readonly Dictionary<string, Func<Game, SGFProperty, Game>> PropertyHandlers = new Dictionary<string, Func<Game, SGFProperty, Game>>();
        ...
        protected Game(Game fromGame) {
            ...
            foreach (var p in fromGame._superKoSet) {
                _superKoSet.Add(p);
            }
            Root = fromGame.Root;
        }
        ...
        public Game MakeMove(int x, int y) {
            return MakeMove(x, y, out var dummy);
        }
        ...
        public Game Pass() {
            var g = new Game(this);
            _moves.Add(new Variation(PassMove, g));
            return g;
        }
        ...
        protected bool InternalMakeMove(int x, int y) {
            ...
            if (Board[x, y] != Content.Empty) {
                // This is an illegal overwrite move
                legal = false;
            }
            Board[x, y] = oturn;
            Move = new Point(x, y);
            ...
            if (_superKoSet != null) {
                if (_superKoSet.Contains(Board, SuperKoComparer)) {
                    // Violates super-ko
                    legal = false;
                }
